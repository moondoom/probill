...
from zpayment.models import PrePayment, Payment, Invoice, Purse
from billing.views import sub_auth
...
def success(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            response['message'] = 'Платёж завершился Успешно!'
    return response
...
def fail(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            response['message'] = 'Платёж завершился с ошибкой!'
